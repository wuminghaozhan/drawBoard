...
**固定图层说明：**
- **background层** (z-index: 0) - 背景、网格等静态内容
- **draw层** (z-index: 1) - 所有虚拟图层的内容都绘制在这个层上
- **interaction层** (z-index: 2) - 全局交互元素（实时预览、临时选择框等）
...
为了解决选中虚拟图层的交互元素遮挡上层图层的问题，在**3个固定物理图层**基础上，实现了动态物理图层系统：

**基础架构（固定3层）：**
```
interaction层 (z-index: 2) ← 全局交互（实时预览、临时选择框）
draw层 (z-index: 1)        ← 所有虚拟图层内容（都绘制在这个层上）
...
**重要说明：**
- 所有虚拟图层的内容都绘制在同一个 `draw层` 上，通过虚拟图层的 `zIndex` 控制绘制顺序
...
**职责：**
- 管理**3个固定的物理Canvas层**（background、draw、interaction）
- 处理Canvas尺寸和上下文管理
- 提供图层访问接口
- 支持**动态图层创建和管理**（按需创建，用于选中图层的交互元素）

**固定图层：**
- `background` (z-index: 0) - 背景层
- `draw` (z-index: 1) - 绘制层（所有虚拟图层内容都绘制在此）
- `interaction` (z-index: 2) - 交互层（全局交互元素）

**动态图层：**
- 按需创建，用于选中虚拟图层的交互元素（锚点、控制点等）
- zIndex计算公式：`virtualLayerZIndex * 10 + 5`
- 图层取消选中时自动删除
- **注意**：动态图层只用于交互元素，不用于绘制虚拟图层内容
- **虚拟图层内容绘制**：所有虚拟图层内容都绘制在同一个 `draw层` 上，通过离屏Canvas缓存优化性能
